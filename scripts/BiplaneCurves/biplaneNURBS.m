...
if root_joint_flag
    ...
    fid = fopen([output_path 'BC_rootTrans_upper_curve.dat'], 'wt');
    ...
    % write NURBS curve to DYMORE-formatted file
    fid = fopen([output_path 'BG_rootTrans_lower_curve.dat'], 'wt');
    fprintf(fid, '@CURVE_DEFINITION {\n');
...
% write NURBS curve to DYMORE-formatted file
fid = fopen([output_path 'DE_jointTrans_upper_curve.dat'], 'wt');
fprintf(fid, '@CURVE_DEFINITION {\n');
...
% write NURBS curve to DYMORE-formatted file
fid = fopen([output_path 'HE_jointTrans_lower_curve.dat'], 'wt');
fprintf(fid, '@CURVE_DEFINITION {\n');
...
%         plot([x1(j) x1(j)], [s+g/2 -s+g/2], 'kx-');
%         plot([x1(j) x1(j)], [s-g/2 -s-g/2], 'kx-');
%     elseif jt_beg_station < j && j < jt_end_station
%         % fprintf('station #%d, joint transition region\n', j)
%         % upper station
%         [x, y, curvature, tang_x, tang_y, norm_x, norm_y] = get_curvatures_tangents_normals(jointTrans_upper, [x1_to_eta(jointTrans_upper,x1(j))], 0, 0);
%         plot([x(1) x(1)+s*norm_x(1)], [y(1) y(1)+s*norm_y(1)], 'kx-')
%         plot([x(1) x(1)-s*norm_x(1)], [y(1) y(1)-s*norm_y(1)], 'kx-')
%         % lower station
%         [x, y, curvature, tang_x, tang_y, norm_x, norm_y] = get_curvatures_tangents_normals(jointTrans_lower, [x1_to_eta(jointTrans_lower,x1(j))], 0, 0);
%         plot([x(1) x(1)+s*norm_x(1)], [y(1) y(1)+s*norm_y(1)], 'kx-')
%         plot([x(1) x(1)-s*norm_x(1)], [y(1) y(1)-s*norm_y(1)], 'kx-')
%     else
%         % fprintf('station #%d, outboard monoplane region\n', j)
%         % plot([x1(j) x1(j)], [cs_heights(j)/2 -cs_heights(j)/2], 'k-');
%         plot([x1(j) x1(j)], [s -s], 'kx-');
%     end
% end

for j=1:length(x1)
    s = cs_heights(j)/2;  % half-height scaling factor for biplane cross-sections
                          % ...(Hawaii style; monoplane and biplane cross-sections have the same height, but different wall thicknesses)
    if root_joint_flag
        if j <= rt_beg_station
            plot([x1(j) x1(j)], [s -s], 'kx-');
        elseif rt_beg_station < j && j < rt_end_station
            % upper station
            [x, y, curvature, tang_x, tang_y, norm_x, norm_y] = get_curvatures_tangents_normals(rootTrans_upper, [x1_to_eta(rootTrans_upper,x1(j))], 0, 0);
            plot([x(1) x(1)+s*norm_x(1)], [y(1) y(1)+s*norm_y(1)], 'kx-')
            plot([x(1) x(1)-s*norm_x(1)], [y(1) y(1)-s*norm_y(1)], 'kx-')
            % lower station
            [x, y, curvature, tang_x, tang_y, norm_x, norm_y] = get_curvatures_tangents_normals(rootTrans_lower, [x1_to_eta(rootTrans_lower,x1(j))], 0, 0);
            plot([x(1) x(1)+s*norm_x(1)], [y(1) y(1)+s*norm_y(1)], 'kx-')
            plot([x(1) x(1)-s*norm_x(1)], [y(1) y(1)-s*norm_y(1)], 'kx-')
