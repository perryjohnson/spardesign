...
data = np.loadtxt('testmatrix_rootBendMom_results_full-hSW.txt')

# create a dictionary of keys for each column in the array "data"
colDict = {'r_j/R'            : 0,
           'g/c'              : 1,
           'max root bending moment' : 2}
...
z = data[:,colDict['max root bending moment']]
...
plt.clabel(CS, inline=1, fmt='%3.1e', fontsize=11)  # write the z-values on the contour lines
# plt.clabel(CS2, inline=1, fmt='%5.4f', fontsize=11)  # write the z-values on the contour lines

plt.subplots_adjust(left=0.12, bottom=0.12, right=0.92, top=0.93, wspace=0.20, hspace=0.20)
cbar = plt.colorbar(CSF, format='%3.1e', spacing='proportional')  # plot a colorbar legend
cbar.set_label('max root bending moment [N*m]')
...
plt.savefig('constlddist_rootBendMom_May2012_full-hSW.png')
plt.savefig('constlddist_rootBendMomMay2012_full-hSW.pdf')
