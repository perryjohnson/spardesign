...
# spar_stn_list = [22, 23, 24]  # generate [M] and [K] matrices for these spar stations
...
# store extrapolated results from DYMORE (monoplane_spar_constload/untwisted/grid_density_1/FIGURES/svy_force_spar_new.mdt)
...
for n in range(len(spar_stn_list)):
    ...
    recovery_dict['F1'] = F1[spar_station-1]  # take the row for the current spar station; index 0 has data for station 1
    recovery_dict['F2'] = F2[spar_station-1]
    recovery_dict['F3'] = F3[spar_station-1]
    recovery_dict['M1'] = M1[spar_station-1]
    recovery_dict['M2'] = M2[spar_station-1]
    recovery_dict['M3'] = M3[spar_station-1]
